...
```
# v1.1.2 기준
- 통화 중 끊김 현상 및 흰색 화면으로 보이는 현상
- CCC 앱에서 간헐적으로 화면이 깜빡거리는 현상 수정
- 비디오 OFF → ON 변경시에 검은색 화면에서 반짝이듯이 출력 되는 현상
- 회의 중 앱을 background로 전환 시 약 1분(기기마다 상이) 경과되면 Local 카메라 정지되는 현상
- iPad에서 본인의 카메라 정보를 불러올 수 없고, 웹 브라우저 선택 팝업이 출력되지 않는 현상
- 모바일 접속 시 통화 방에서 다른 회의 참여 시 비정상 동작 (버튼 임시 삭제)
- 모바일 웹 브라우저에서 팝업 등의 액션이 뜬 상태에서 단말의 뒤로가기 버튼 터치 시 메인 화면으로 이동 (정책 문제로 보류)
- 화면 레이아웃 변경이 스무스 하지 않은 경우가 있음 (room에 user count 부분 / changeview 부분 리뷰)
